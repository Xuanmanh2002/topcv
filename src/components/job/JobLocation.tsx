...
const JobLocation = () => {
    ...
    const getAddressName = () => {
        ...
        if (isNaN(numericAddressId)) return "Địa chỉ không xác định";

        const address = addresses.find((ad) => ad.id === numericAddressId);
        return address ? address.name : "Địa chỉ không xác định";
    };

    useEffect(() => {
        const fetchJobs = async () => {
            try {
                const data = await getAllJobsByAddress(Number(addressId));
                setJobs(data);
                setLoading(false);
            } catch (err: any) {
                        ...
                        <div className='job-search-navbar-title'>
                            ...
                            <div className='job-search-navbar-link'>
                                <ul className='job-search-navbar-item'>
                                    ...
                                    <FaChevronRight className='job-search-icon-right' />
                                    <p className='job-search-icon-describe' style={{ paddingLeft: "10px" }} >{getAddressName()}</p>
                                </ul>
                            </div>
                        </div>
